...
<head>
    ...
    <style>
        ...
        .slot-free a { color: #155724; font-weight: 600; text-decoration: none; }
        .slot-free a:hover { text-decoration: underline; }
...
<div class="container">
    <header>
        <h1>Free Slots</h1>
        <div>
            <a href="reservation_form.php" class="link" style="color:#ffc107;">Reservation form</a>
            &nbsp;|&nbsp;
            <a href="my_reservations.php" class="link" style="color:#ffc107;">My Reservations</a>
            &nbsp;|&nbsp;
            <a href="../Login/logout.php" class="link" style="color:#ffc107;">Sign out</a>
        </div>
    </header>
    <main>
        <p style="margin:0 0 12px 0; font-size:0.95rem;">Select a date (required). Optionally filter by building and hall. You will see 7 days (3 before and 3 after). <strong>Click a date</strong> to see available time slots for that day. Click a <strong>free</strong> slot to open the reservation form with that start time.</p>
        ...
        <?php if ($selectedDate && count($halls) > 0): ?>
            ...
            <?php if ($viewDay): ?>
                ...
                <p style="margin:0 0 8px 0; font-size:0.9rem;">Click a <strong>Free</strong> slot to open the reservation form with that start time.</p>
                        ...
                        <?php foreach ($halls as $hall): ?>
                            <tr>
                                ...
                                foreach ($slotHours as $i => $hour):
                                    ...
                                    $startStr = $slotStart->format('Y-m-d\TH:i');
                                    $endStr = $slotEnd->format('Y-m-d\TH:i');
                                    $url = $formUrl . '?start=' . urlencode($startStr) . '&end=' . urlencode($endStr) . '&hall_id=' . (int)$hall['hall_id'] . '&building_id=' . (int)$hall['building_id'];
                                ?>
                                    <td class="<?php echo $isBusy ? 'slot-busy' : 'slot-free'; ?>">
                                        <?php if ($isBusy): ?>
                                            Busy
                                        <?php else: ?>
                                            <a href="<?php echo htmlspecialchars($url); ?>">Free</a>
                                        <?php endif; ?>
                                    </td>
                                <?php endforeach; ?>
                            </tr>
